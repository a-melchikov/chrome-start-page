...
Применять theme-aware Liquid Glass к карточкам Markdown, часов, Pomodoro,
компактной погоды и пустого изображения, поверхности Search и верхней панели
управления в компактном и расширенном состоянии.
Визуальная погода и загруженное изображение получают только кромку и тень,
без стеклянной подложки поверх сцены.
Эффект включён по умолчанию, хранится в
`appearance.liquidGlass` схемы v5 и настраивается в закрытом разделе «Виджеты»
окна оформления. Общие для всех тем параметры: прозрачность `0–100%`,
...
ровную полупрозрачную подложку, тонкую внутреннюю кромку и короткую мягкую
тень. Корневые custom properties не меняют схему хранения; панель управления
использует ту же плотность подложки, что и карточки, а диалоги остаются
непрозрачными.
...
`backdrop-filter`; погодная сцена и изображение сохраняют исходные цвета.
Без поддержки фильтра остаётся CSS-fallback. Миграции
...
- Применение стекла к диалогам и служебным панелям помимо верхнего toolbar.
...
Glass. The architecture introduces `themes/` with a typed registry and 13
curated presets (`system`, `light`, `dark`, `tokyo-night`, `rainy-tokyo`,
`cozy-lofi-night`, `catppuccin-mocha`, `catppuccin-latte`, `nord`,
`synthwave-84`, `solarized-dark`, `paper-sage`, `apricot-noon`).

«Бумага и шалфей» и «Абрикосовый полдень» добавлены как отдельные тёплые
светлые пресеты. Они используют существующие семантические токены и механизм
Liquid Glass, не меняя структуру конфигурации v5.
...
`Shift+стрелка` двигает группу до ближайшей границы или препятствия; для
движения вниз границей служит низ видимой области, поскольку сетка открыта
вниз.
...
## ADR-014 — Система собственных тем и детерминированный расчёт цветов

Статус: принято

### Решение

Ввести поддержку собственных тем пользователя на базе схемы v6 (`version: 6`).
Темы хранятся в `config.customThemes: CustomTheme[]`, а выбранная тема задаётся
объектом `appearance.theme: ThemeRef` (`{ type: 'builtin', id }` или `{ type: 'custom', id }`).
Схема цветов темы охватывает 28 семантических ролей (включая Liquid Glass).
В полноэкранном редакторе темы пользователь задаёт 6 основных цветов
(`canvasBg`, `surfaceBg`, `textPrimary`, `textSecondary`, `accent`, `accentText`),
а остальные 22 вторичные роли рассчитываются детерминированно с возможностью ручной
фиксации («Вручную» / «Вернуть авто»). Предупреждения о контрасте рассчитываются по
алгоритму WCAG, не блокируя сохранение. Применение собственной темы осуществляется
через динамические inline CSS-переменные `--theme-*` на корневом элементе дашборда.

### Причина

Пользователи получают неограниченные возможности персонализации палитры с сохранением
целостности интерфейса всех типов виджетов и читаемости текста без необходимости
ручной настройки каждого служебного оттенка с нуля.

### Последствия

Схема конфигурации повышена до v6. Все старые конфигурации v1–v5 автоматически
мигрируют при загрузке. Бэкапы v2 полностью совместимы и переносят массив `customThemes`.
Для встроенных тем сохраняются CSS-классы и селекторы в `themes.css`, а для
собственных тем генерируется полный набор CSS-переменных по маппингу токенов.

### Отклонённые альтернативы

- Хранение только 6 цветов без возможности тонкой настройки зависимых 22 ролей.
- Блокировка сохранения темы при недостаточном контрасте по WCAG (вызывает фрустрацию у пользователей художественных палитр).
- Применение тем через генерацию динамических тегов `<style>`.
