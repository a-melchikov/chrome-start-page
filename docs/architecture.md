...
Перетаскивание не ограничено текущей высотой grid-контейнера: пользователь
может поместить виджет в свободную строку ниже остальных, после чего auto-size
увеличивает высоту canvas. Столкновения с уже занятыми ячейками по-прежнему
блокируются.
